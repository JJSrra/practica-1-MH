...
void evaluarAGGBLX(int bd_elegida, int particion, normal_distribution<double>& distribucion_normal, default_random_engine& generador){
  ...
  cout << "% AGGBLX train en " << nombre_bd << " para partición " << particion << ": " << porcentaje << endl;
  cout << "\tTiempo: " << transcurrido.count() << endl;

  t1 = chrono::high_resolution_clock::now();
  AGGBLX(test, etiquetas_test, pesos_calculados, distribucion_normal, generador);
  t2 = chrono::high_resolution_clock::now();
  ...
  cout << "% AGGBLX test en " << nombre_bd << " para partición " << particion << ": " << porcentaje << endl;
  cout << "\tTiempo: " << transcurrido.count() << endl;
}
